...
// Operational bridge for one accepted M2 Work Item. It preserves Grok's
// synchronous foreground Goal Planner and Task-local plan mutators while
// removing only the unused background controls that made Planner startup fail.
// The product Workspace remains read-only to Grok model tools. This is not the
// integrated ForkLight native-goal adapter.
...
const BACKGROUND_CONTROLS = new Set(["get_task_output", "kill_task", "wait_tasks"]);
...
export function buildForegroundPlanInvocation(input) {
  ...
  if (!prompt || !sessionId || !grokHome) {
    throw new Error("ForkLight foreground-plan bootstrap is missing prompt, session id, or GROK_HOME.");
  }
  if (promptIndex < 0) {
    throw new Error("ForkLight foreground-plan bootstrap requires headless -p mode.");
  }
  ...
  replaceCommaList(argv, "--tools", (entries) => Array.from(new Set([
    ...entries.filter((entry) => entry !== "run_terminal_cmd" && !BACKGROUND_CONTROLS.has(entry)),
    "search_replace",
    "write",
    "Agent",
  ])));
...
function main() {
  ...
  if (sessionId && grokHome) {
    ...
    if (existsSync(statePath)) {
      try {
        ...
        process.stderr.write("ForkLight foreground-plan bootstrap found unreadable existing Goal state.\n");
        process.exit(65);
      }
    }
  }
  ...
  let invocation;
  try {
    invocation = buildForegroundPlanInvocation({
      argv: rawArgv,
      cwd: process.cwd(),
      grokHome,
      priorStatus,
  ...
  child.on("error", (error) => {
    process.stderr.write(`ForkLight foreground-plan bootstrap could not launch Grok: ${error.message}\n`);
    process.exitCode = 1;
  });
