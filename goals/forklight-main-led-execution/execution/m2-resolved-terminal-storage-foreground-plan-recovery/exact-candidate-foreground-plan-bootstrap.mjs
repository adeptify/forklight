...
const FOREGROUND_BOOTSTRAP = "goals/forklight-main-led-execution/execution/m2-resolved-terminal-storage-foreground-plan-recovery/foreground-plan-bootstrap.mjs";
const cwd = process.cwd();
const seedPath = path.join(cwd, SEED);
const foregroundBootstrapPath = path.join(cwd, FOREGROUND_BOOTSTRAP);
const runtimeEnv = {
  ...process.env,
...
const child = spawn(foregroundBootstrapPath, process.argv.slice(2), {
  cwd,
  env: runtimeEnv,
  stdio: "inherit",
});
...
child.on("error", (error) => fail("Grok foreground-plan bootstrap failed to launch", error.message));
