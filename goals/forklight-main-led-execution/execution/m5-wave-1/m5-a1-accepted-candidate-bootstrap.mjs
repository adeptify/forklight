...
// Operational bootstrap for the M5-A1 accepted-Candidate delivery replacement. The
// exact already-accepted Candidate is part of this Task's source snapshot, so the
...
const SEED_RELATIVE = "goals/forklight-main-led-execution/execution/m5-wave-1/m5-a1-accepted-candidate.patch";
const MARKER_RELATIVE = ".forklight/m5-a1-accepted-candidate-seed.json";
...
function seedPaths(seedText) {
  ...
  for (const match of seedText.matchAll(
    ...
    if (match[1] !== match[2]) fail("M5-A1 accepted Candidate seed contains a renamed path");
    paths.push(match[2]);
  }
  return paths;
}
...
function materializeWorkspaceLocalSeed(workspace) {
  ...
  try {
    ...
    fail("M5-A1 accepted Candidate seed is missing or unreadable", error instanceof Error ? error.message : String(error));
  }
  const paths = seedPaths(seedText);
  if (!exactPathSet(paths)) fail("M5-A1 accepted Candidate seed path set does not match the accepted 16 paths");

  const markerPath = path.join(workspace, MARKER_RELATIVE);
  if (!existsSync(markerPath)) {
    const forward = gitApply(workspace, seedPath, ["--check"]);
    if (forward.status !== 0) {
      fail("M5-A1 accepted Candidate seed does not match this Task source base", forward.stderr);
    }
    const applied = gitApply(workspace, seedPath, []);
    if (applied.status !== 0) fail("M5-A1 accepted Candidate seed could not be materialized", applied.stderr);
  ...
  try {
    ...
    fail("M5-A1 accepted Candidate seed marker is unreadable", error instanceof Error ? error.message : String(error));
  }
  if (marker.seed !== SEED_RELATIVE || !Array.isArray(marker.paths) || !exactPathSet(marker.paths)) {
    fail("M5-A1 accepted Candidate seed marker does not match the accepted seed");
  }
}
...
) fail("M5-A1 accepted-Candidate replacement requires ForkLight's current-model-only native Goal invocation");
...
child.on("error", (error) => {
  process.stderr.write(`M5-A1 accepted-Candidate replacement could not launch Grok: ${error.message}\n`);
  process.exitCode = 1;
});
