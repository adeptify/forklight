...
// Operational bootstrap for the explicitly authorized one-Judge A1Q delivery.
// The exact verified Candidate is copied into this Task's source snapshot and
// materialized before Grok starts. No prior Task path is read after admission.
...
const SEED_RELATIVE = "goals/forklight-main-led-execution/execution/m5-wave-1/m5-a1q-bootstrap-exact-candidate.patch";
const MARKER_RELATIVE = ".forklight/m5-a1q-bootstrap-exact-candidate-seed.json";
...
function seedPaths(seedText) {
  ...
  for (const match of seedText.matchAll(
    ...
    if (match[1] !== match[2]) fail("M5-A1Q bootstrap seed contains a renamed path");
    paths.push(match[2]);
  }
  return paths;
}
...
function materializeWorkspaceLocalSeed(workspace) {
  ...
  try {
    ...
    fail("M5-A1Q bootstrap seed is missing or unreadable", error instanceof Error ? error.message : String(error));
  }
  const paths = seedPaths(seedText);
  if (!exactPathSet(paths)) fail("M5-A1Q bootstrap seed path set does not match the verified three paths");

  const markerPath = path.join(workspace, MARKER_RELATIVE);
  if (!existsSync(markerPath)) {
    const forward = gitApply(workspace, seedPath, ["--check"]);
    if (forward.status !== 0) {
      fail("M5-A1Q bootstrap seed does not match this Task source base", forward.stderr);
    }
    const applied = gitApply(workspace, seedPath, []);
    if (applied.status !== 0) fail("M5-A1Q bootstrap seed could not be materialized", applied.stderr);
  ...
  try {
    ...
    fail("M5-A1Q bootstrap seed marker is unreadable", error instanceof Error ? error.message : String(error));
  }
  if (marker.seed !== SEED_RELATIVE || !Array.isArray(marker.paths) || !exactPathSet(marker.paths)) {
    fail("M5-A1Q bootstrap seed marker does not match the verified seed");
  }
}
...
) fail("M5-A1Q bootstrap requires ForkLight's current-model-only native Goal invocation");
...
child.on("error", (error) => {
  process.stderr.write(`M5-A1Q bootstrap could not launch Grok: ${error.message}\n`);
  process.exitCode = 1;
});
