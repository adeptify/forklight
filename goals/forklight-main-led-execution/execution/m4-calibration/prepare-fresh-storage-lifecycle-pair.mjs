...
const inputSource = path.join(here, "inputs", "forklight-storage-lifecycle-checkpoint.json");
...
if (
  ...
  || input.taskClass !== "m4-fresh-checkpoint-storage-lifecycle"
) {
  throw new Error("fresh storage-lifecycle comparison input identity is invalid");
}
if (!input.outputPath.endsWith("/forklight-storage-lifecycle-checkpoint.json")) {
  throw new Error("fresh storage-lifecycle comparison output path is invalid");
}

const pairRoot = await mkdtemp(path.join(tmpdir(), "forklight-m4e-storage-lifecycle-pair-"));
const roots = {};
