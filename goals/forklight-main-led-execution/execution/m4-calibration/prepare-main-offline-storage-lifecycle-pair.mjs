...
const inputSource = path.join(
  here,
  "inputs",
  "forklight-storage-lifecycle-main-offline-checkpoint.json",
);
...
if (
  ...
  || input.taskClass !== "m4-main-offline-storage-lifecycle-checkpoint"
) {
  throw new Error("Main-offline storage-lifecycle comparison input identity is invalid");
}
if (!input.outputPath.endsWith("/forklight-storage-lifecycle-main-offline-checkpoint.json")) {
  throw new Error("Main-offline storage-lifecycle output path is invalid");
}

const pairRoot = await mkdtemp(path.join(tmpdir(), "forklight-m4e-main-offline-storage-pair-"));
const roots = {};
