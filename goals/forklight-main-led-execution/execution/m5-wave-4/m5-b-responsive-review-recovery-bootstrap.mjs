...
// Operational bootstrap for the bounded M5-B responsive-review Candidate recovery.
...
const SEED_RELATIVE = "goals/forklight-main-led-execution/execution/m5-wave-4/m5-b-responsive-review-candidate.patch";
const MARKER_RELATIVE = ".forklight/m5-b-responsive-review-seed.json";
...
function seedPaths(seedText) {
  ...
  for (const match of seedText.matchAll(
    ...
    if (match[1] !== match[2]) fail("M5-B responsive-review Candidate contains a renamed path");
    paths.push(match[2]);
  }
  return paths;
}
...
function materializeWorkspaceLocalSeed(workspace) {
  ...
  try {
    ...
    fail("M5-B responsive-review Candidate is missing or unreadable", error instanceof Error ? error.message : String(error));
  }
  const paths = seedPaths(seedText);
  if (!exactPathSet(paths)) fail("M5-B responsive-review Candidate path set is not the accepted seven paths");

  const markerPath = path.join(workspace, MARKER_RELATIVE);
  if (!existsSync(markerPath)) {
    const forward = gitApply(workspace, seedPath, ["--check"]);
    if (forward.status !== 0) {
      fail("M5-B responsive-review Candidate does not match this Task source base", forward.stderr);
    }
    const applied = gitApply(workspace, seedPath, []);
    if (applied.status !== 0) fail("M5-B responsive-review Candidate could not be materialized", applied.stderr);
  ...
  try {
    ...
    fail("M5-B responsive-review Candidate marker is unreadable", error instanceof Error ? error.message : String(error));
  }
  if (marker.seed !== SEED_RELATIVE || !Array.isArray(marker.paths) || !exactPathSet(marker.paths)) {
    fail("M5-B responsive-review Candidate marker does not match the accepted seed");
  }
}
...
) fail("M5-B recovery requires ForkLight's final responsive current-model-only native Goal invocation");
