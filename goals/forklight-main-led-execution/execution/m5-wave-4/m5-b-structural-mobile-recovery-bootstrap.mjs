...
// Operational bootstrap for the M5-B structural mobile-hierarchy Candidate recovery.
...
const MARKER_RELATIVE = ".forklight/m5-b-structural-mobile-seed.json";
...
function seedPaths(seedText) {
  ...
  for (const match of seedText.matchAll(
    ...
    if (match[1] !== match[2]) fail("M5-B structural-mobile Candidate contains a renamed path");
    paths.push(match[2]);
  }
  return paths;
}
...
function materializeWorkspaceLocalSeed(workspace) {
  ...
  try {
    ...
    fail("M5-B structural-mobile Candidate is missing or unreadable", error instanceof Error ? error.message : String(error));
  }
  const paths = seedPaths(seedText);
  if (!exactPathSet(paths)) fail("M5-B structural-mobile Candidate path set is not the accepted seven paths");

  const markerPath = path.join(workspace, MARKER_RELATIVE);
  if (!existsSync(markerPath)) {
    const forward = gitApply(workspace, seedPath, ["--check"]);
    if (forward.status !== 0) {
      fail("M5-B structural-mobile Candidate does not match this Task source base", forward.stderr);
    }
    const applied = gitApply(workspace, seedPath, []);
    if (applied.status !== 0) fail("M5-B structural-mobile Candidate could not be materialized", applied.stderr);
  ...
  try {
    ...
    fail("M5-B structural-mobile Candidate marker is unreadable", error instanceof Error ? error.message : String(error));
  }
  if (marker.seed !== SEED_RELATIVE || !Array.isArray(marker.paths) || !exactPathSet(marker.paths)) {
    fail("M5-B structural-mobile Candidate marker does not match the accepted seed");
  }
}
...
) fail("M5-B recovery requires ForkLight's structural-mobile current-model-only native Goal invocation");
...
child.on("error", (error) => {
  process.stderr.write(`M5-B structural-mobile recovery could not launch Grok: ${error.message}\n`);
  process.exitCode = 1;
});
