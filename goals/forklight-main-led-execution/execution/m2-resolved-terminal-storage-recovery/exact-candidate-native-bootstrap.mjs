...
const NATIVE_BOOTSTRAP = "goals/forklight-main-led-execution/execution/m2-grok-native-goal/grok-native-goal-bootstrap.mjs";
...
if (check.status === 0) {
  ...
  if (alreadyApplied.status !== 0) fail("Workspace is neither clean-applicable nor exactly applied", check.stderr);
}
...
child.on("error", (error) => fail("Grok native bootstrap failed to launch", error.message));
