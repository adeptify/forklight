...
// Operational bootstrap for the authorized M3-C2 Workspace-local-seed
// replacement. The exact protected partial is part of the Task source
// snapshot, so this process never reads another Task path after Grok's
// Workspace sandbox is active. This file is not product code.
...
const SEED_RELATIVE = "goals/forklight-main-led-execution/execution/m3-wave-3/m3-c2-protected-partial.patch";
const MARKER_RELATIVE = ".forklight/m3-c2-workspace-local-seed.json";
...
function seedPaths(seedText) {
  ...
  for (const match of seedText.matchAll(
    ...
    if (match[1] !== match[2]) fail("M3-C2 Workspace-local seed contains a renamed path");
    paths.push(match[2]);
  }
  return paths;
}
...
function materializeWorkspaceLocalSeed(workspace) {
  ...
  try {
    ...
    fail("M3-C2 Workspace-local seed is missing or unreadable", error instanceof Error ? error.message : String(error));
  }
  const paths = seedPaths(seedText);
  if (!exactPathSet(paths)) fail("M3-C2 Workspace-local seed path set does not match the authorized 14 paths");

  const markerPath = path.join(workspace, MARKER_RELATIVE);
  if (!existsSync(markerPath)) {
    const forward = gitApply(workspace, seedPath, ["--check"]);
    if (forward.status !== 0) {
      fail("M3-C2 Workspace-local seed does not match this Task source base", forward.stderr);
    }
    const applied = gitApply(workspace, seedPath, []);
    if (applied.status !== 0) fail("M3-C2 Workspace-local seed could not be materialized", applied.stderr);
  ...
  try {
    ...
    fail("M3-C2 Workspace-local seed marker is unreadable", error instanceof Error ? error.message : String(error));
  }
  if (marker.seed !== SEED_RELATIVE || !Array.isArray(marker.paths) || !exactPathSet(marker.paths)) {
    fail("M3-C2 Workspace-local seed marker does not match the authorized seed");
  }
}
...
) fail("M3-C2 replacement requires ForkLight's current-model-only native Goal invocation");
...
child.on("error", (error) => {
  process.stderr.write(`M3-C2 replacement could not launch Grok: ${error.message}\n`);
  process.exitCode = 1;
});
