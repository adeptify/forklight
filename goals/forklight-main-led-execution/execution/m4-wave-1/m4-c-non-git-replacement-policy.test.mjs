...
const wrapper = readFileSync(path.join(directory, "m4-c-non-git-replacement-bootstrap.mjs"), "utf8");
const postState = readFileSync(path.join(directory, "m4-c-non-git-replacement-postapply.test.mjs"), "utf8");
const task = YAML.parse(readFileSync(path.join(directory, "06-m4-c-non-git-post-state-proof-replacement.yaml"), "utf8"));
...
const finalRecoveryWorkspace = "/Users/yijunwang/Library/Application Support/ForkLight/runs/812df915-788c-463c-8ee0-208d71ff9d58/workspace";
const acceptanceReplacementWorkspace = "/Users/yijunwang/Library/Application Support/ForkLight/runs/4a967967-ac35-48e0-b89a-fd1e7c636e5d/workspace";
...
function check(cwd, patchPath, strip, extra = []) {
  const result = spawnSync(
    "/usr/bin/git",
    ["apply", `-p${strip}`, ...extra, "--check", patchPath],
    {
      cwd,
      encoding: "utf8",
...
for (const finalWorkspace of [finalRecoveryWorkspace, acceptanceReplacementWorkspace]) {
...
assert.doesNotMatch(wrapper, /89e60adc|812df915|4a967967|Application Support\/ForkLight\/runs/);
assert.doesNotMatch(wrapper, /lock|lease|version handshake/);

assert.match(postState, /\["apply", `-p\$\{strip\}`, "--reverse", "--check"/);
assert.match(postState, /createHash\("sha256"\)\.update\(base\)/);
assert.match(postState, /matchAll\(/);
assert.match(postState, /assert\.deepEqual\(new Set\(basePaths\), expectedPaths\)/);
assert.doesNotMatch(postState, /\["(?:diff|status|ls-files|rev-parse)"/);
assert.doesNotMatch(postState, /\.git/);
assert.doesNotMatch(postState, /marker|\.forklight/);

assert.equal(task.runtime.name, "grok-build");
assert.equal(task.runtime.effort, "xhigh");
assert.equal(task.worker.allowEdits, false);
assert.deepEqual(task.worker.allowedCommands, []);
...
assert.match(task.acceptance.commands[0], /m4-c-non-git-replacement-postapply\.test\.mjs$/);
...
console.log("M4-C final non-Git replacement policy passed");
