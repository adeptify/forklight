...
// Final M4-C acceptance-contract-only bootstrap. Both accepted patches live in
...
const MARKER_RELATIVE = ".forklight/m4-c-acceptance-replacement.json";
...
function materialize(workspace) {
  ...
  try {
    ...
    fail("M4-C replacement seed is missing or unreadable", error instanceof Error ? error.message : String(error));
  }
  ...
  if ((deltaText.match(/^--- /gm) ?? []).length !== 1 || (deltaText.match(/^\+\+\+ /gm) ?? []).length !== 1) {
  ...
  if (!existsSync(markerPath)) {
    ...
    try {
      ...
      fail("M4-C replacement marker is unreadable", error instanceof Error ? error.message : String(error));
    }
    if (marker.base !== BASE_DIGEST || marker.delta !== DELTA_DIGEST || !exactBasePaths(marker.paths ?? [])) {
      fail("M4-C replacement marker does not match the authorized Candidate");
    }
  }

  assertApply(gitApply(workspace, deltaPath, 1, ["--reverse", "--check"]), "M4-C final test state drifted");
  assertApply(
